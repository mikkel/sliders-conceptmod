...
**Works on some pairs:** `faithful_raw`, `hidden_beta1`, `semantic_kl_null`, `gender_like_no_e`, `pair_odd_midpoint`, `hold_e_perp_l8`, `pair_odd_sub_e`, `faithful_sub_e`, `semantic_kl_midpoint`, `semantic_kl_poles`, `semantic_kl_sub_e`. Each passes at least one pair and fails another. `faithful_raw` (and `hidden_beta1`, which is the same target reached through `--lm_target symmetric --common_beta 1`) passes all three pair cells and is charged only by the unused-ê sheet — which is a leak gender-v4 has no `leak_*` to trip, so it is the next live card. `semantic_kl_poles` is the row the live exam is about: the energy win and the gender garble.
...
- `hidden_beta1` — pair-odd β=1 / symmetric --common_beta 1. lm_hidden_targets(symmetric, β=1) is the raw poles. Sheet-good, still leaks ê. Fixture: sheet leftover faithful ≡ β=1 + gender hidden_beta1.
  ...
  - close pair: on-continuation
  - unused_e pair: on-continuation
- `semantic_kl_null` — semantic_kl + null-space hidden pin. faithful caption poles; KL on the readout row space plus hidden MSE on ker(W) pins delivery the KL cannot see. Fixture: pair-exam + sheet leftover + sheet gender.
  - divergent pair: on-continuation
  - close pair: on-continuation
  - unused_e pair: on-continuation
...
## The KL fix the pair-exam cell found

`semantic_kl_null` — `--lm_target faithful --pole_mode semantic_kl_null` —
keeps the real caption poles and adds hidden MSE on `ker(lm_head)`: the
part of the state semantic KL has zero gradient on. On the pair-exam cell
it passes **both** divergent and close pairs (`exam_score` 0.994) while
plain `semantic_kl_poles` still garbles gender. It is not a rename of
`faithful_raw`: the pole term is still KL on the semantic band; only the
null-space pin is hidden MSE.
...
What to watch: `invisible_kept` should move from ~0 (v16) toward 1, and
the listen should keep the pole's swing. `c+` will still look like a KL
recipe, not v9's pair-odd lock — that is expected.
