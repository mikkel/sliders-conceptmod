...
# Partial keep of the common component. β=0 is pair-odd (banned: energy
# sings a blend). β=1 is the raw poles (passes divergent, leak_frac ≥ 0
# on energy-v4). The exam is whether any β in (0, 1) still passes
# divergent with leftover_bipolar leak_frac < 0.
COMMON_BETA_GRID = (
    0.0,
    0.05,
    0.08,
...
def common_beta_sweep(
    grid: tuple[float, ...] = COMMON_BETA_GRID,
    *,
    steps: int = 400,
    seed: int = 0,
) -> list[dict]:
    """``tgt = (1−β)·(h0 ± a) + β·h±`` on the pair-exam cells.

    Live that is ``--lm_target symmetric --common_beta β`` /
    ``lm_hidden_targets(..., common_beta=β)``. ``--lm_target v9``
    hard-codes β=0 and ignores the flag. This sweep does not change
    the live default.
    """
    fields = {name: CELLS[name](seed=seed) for name in ("divergent", "close", "unused_e")}
    out = []
    for beta in grid:
        scored = {}
        for name, field in fields.items():
            scored[name] = score_exam(
        ...
        divergent = scored["divergent"]
        close = scored["close"]
        unused = scored["unused_e"]
        out.append(
            {
                "common_beta": float(beta),
                ...
                "leftover_leak": unused["leak_tok"],
                "divergent_overlap": divergent["roll_overlap"],
                "divergent_match": divergent["roll_match_kept"],
                "divergent_coherence": divergent["roll_coherence"],
                "divergent_off": divergent["roll_off_corpus"],
                "divergent_swing": divergent["roll_swing_kept"],
                "divergent_reason": divergent["reason"],
                "divergent_near_gate": list(divergent["near_gate"]),
                "close_overlap": close["roll_overlap"],
                "close_match": close["roll_match_kept"],
                "close_swing": close["roll_swing_kept"],
                "close_reason": close["reason"],
                "close_near_gate": list(close["near_gate"]),
                "close_leak_frac": float(close["leak_frac"]),
                "unused_leak_frac": float(unused["leak_frac"]),
                "blend_teacher": bool(divergent["blend_teacher"]),
                "is_caption": bool(divergent["is_caption"]),
            }
        )
    return out
