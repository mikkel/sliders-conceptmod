...
def plot_compare(results: list[MethodResult], path: Path) -> None:
    ...
    styles = {
        ...
        "lm_v9": ("#1e8449", "lm_v9 (odd + hold ê, κ=0)"),
        "lm_v9_project": ("#7f8c8d", "lm_v9_project (short-û)"),
...
def write_report(results: list[MethodResult], geo: dict, blob: dict, path: Path) -> None:
    ...
    lines += [
        ...
        f"- `lm_v9` (default): κ=0 (no even blend-back) + full odd teacher",
        f"  + hold along unused-attr ê (λ=8). Leak {v9.metrics['leak_ratio']:.3f},",
        f"  ±1 {v9.metrics['cos_plus_minus']:.3f},",
