...
def write_report(
    ...
    lines += [
        ...
        "",
        "Gender: the v4 structured poles are already a clean pair. Do **not**",
        "project onto the short `slider_positive` / `slider_negative`",
        "sentences — live gender-v1 did that (`odd·û/||odd|| = 0.20`) and",
        "hold ate the singer (`||d+||` tiny, cos to the pair 0.20). Train",
        "Hub / symmetric-on-the-pair (`--lm_target symmetric` or",
        "`--lm_target hub`), or pass `--project_align_min 0.50` so v9 falls",
        "back when the short û barely overlaps the poles. Energy: the",
        "structured poles are leaky (mix / genre / BPM junk) and the short",
        "loud/calm û *is* the intended axis; on the 2-D leak cell it is",
        f"{leak_align:.2f}-aligned, so keep `--lm_target v9` project+hold.",
        "**`--lm_target v9` still always-projects by default.** The gate is",
        "opt-in until a live energy run with a *low* alignment is scored.",
        "",
        "## How to run",
        "",
        "```bash",
        "PYTHONPATH=. python analysis/slider2d/run_lm_mismatch.py --out docs/lm-v9-mismatch",
        "PYTHONPATH=. pytest tests/test_lm_v9_mismatch.py tests/test_lm_v9_2d.py tests/test_lm_trainer_v9.py -q",
        "```",
        "",
