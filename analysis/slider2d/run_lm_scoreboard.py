...
def write_report(rows: list[dict], blob: dict, path: Path) -> None:
    ...
    lines += [
        ...
        "one `--lm_target` for gender-v4 and energy-v4, not a typed pair of",
        "them — and it is the only row on this board that passes every cell",
        "without rewriting a caption.",
        "",
        "Being explicit about what is and is not new in its `exam_score`: on",
        "*both* scored pairs the guard ends up at the raw caption (it refuses",
        "on energy-v4 and gender-v4 declares no `ê`), so the fit it is scored",
        "on there is the one `faithful_raw` makes, and the 1.000 is the same",
        "1.000. That is the point rather than a caveat — being safe on a",
        "divergent pair *means* landing where the caption teacher lands. What",
        "the guard changes is the two cells where `ê` is a real leftover: the",
        "unused-ê sheet, where it turns `faithful_raw`'s +0.228 leak into",
        "+0.000, and the `unused_e` pair, where it fits the ê-cleaned target.",
        "The row that changes the *scored* fit is `dual_band`.",
        "",
        ...
        "",
        "It carries the sharper caveat of the two. The blind band here is a",
        "zero column, and a real `lm_head` band reads almost every direction a",
        "little. In the faint-readout sweep on the pair-exam page, the moment",
        "the column stops being exactly zero `--blind_cut 0` finds nothing and",
        "the loss is *exactly* `semantic_kl` again — same swing, same failure.",
        "A cut above that direction's own relative singular value recovers it.",
        "Which cut a real band needs is a property of that band, and no CPU",
        "fixture can supply the number.",
        "",
