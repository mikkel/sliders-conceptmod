...
def write_report(rows: list[dict], blob: dict, path: Path) -> None:
    ...
    if gender_only:
        ...
        lines += [
            ...
            "and fails another. `faithful_raw` / `hidden_beta1` / "
            "`semantic_kl_pin` / `unrolled_kl` pass both live exam pairs "
            "and are charged only by the unused-ê sheet — a leak gender-v4 "
            "has no `leak_*` to trip. `semantic_kl_pin` is the new live "
            "card: same caption target as energy-v18, plus a pin on the "
            "block gender-v16 never moved. `semantic_kl_poles` is the row "
            "the live exam is about: the energy win and the gender garble.",
        ]
    lines += [
        "",
